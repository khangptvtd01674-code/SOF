...
const DEFAULT_SCORES = [
  { groupId: 1, progress: 7.5, report: 7.0, code: 0, presentation: 0, comments: "Tiến độ tốt, giao diện bắt mắt nhưng cần hoàn thiện mã nguồn JS sớm.", finalGrade: 2.9, approved: false },
  { groupId: 2, progress: 10, report: 9.5, code: 9.0, presentation: 9.5, comments: "Đồ án rất xuất sắc, thiết kế chuyên nghiệp, thuyết trình thuyết phục.", finalGrade: 9.5, approved: true }
];
...
const DB = {
  ...
  init() {
    ...
    if (!localStorage.getItem("poly_submissions")) {
      localStorage.setItem("poly_submissions", JSON.stringify([
        {
          id: 1,
          groupId: 1,
          filename: "Báo cáo chương 1.docx",
          time: "1 ngày trước",
          status: "Approved",
          reviewerLogs: ["Sinh viên đã nộp bài", "Giảng viên Nguyễn Thị Minh đã duyệt", "Hội đồng Trần Trung Kiên đã duyệt"]
        },
        {
          id: 2,
          groupId: 1,
          filename: "Báo cáo chương 2.docx",
          time: "3 giờ trước",
          status: "LecturerApproved",
          reviewerLogs: ["Sinh viên đã nộp bài", "Giảng viên Nguyễn Thị Minh đã duyệt"]
        }
      ]));
    }
    // Upgrade existing scores in local storage to support scale of 10 and "approved" field if needed
    try {
      let storedScores = JSON.parse(localStorage.getItem("poly_scores") || "[]");
      let upgraded = false;
      storedScores = storedScores.map(sc => {
        if (sc.approved === undefined) {
          sc.approved = (sc.groupId === 2); // Default group 2 as approved, group 1 as pending
          if (sc.progress > 10) sc.progress /= 10;
          if (sc.report > 10) sc.report /= 10;
          if (sc.code > 10) sc.code /= 10;
          if (sc.presentation > 10) sc.presentation /= 10;
          if (sc.finalGrade > 10) sc.finalGrade /= 10;
          upgraded = true;
        }
        return sc;
      });
      if (upgraded) {
        localStorage.setItem("poly_scores", JSON.stringify(storedScores));
      }
    } catch (e) {
  ...
  load() {
    ...
    this.submissions = JSON.parse(localStorage.getItem("poly_submissions") || "[]");
  },
  ...
  save() {
    ...
    localStorage.setItem("poly_submissions", JSON.stringify(this.submissions));
  }
};
        ...
        <div class="glass-card lecturer-card">
          ...
          <div style="display:flex; gap:10px; flex-wrap:wrap">
            ...
            ${AppState.currentRole === 'admin' ? `
              <button class="btn btn-secondary btn-small" style="border-color:var(--danger); color:var(--danger)" onclick="deleteLecturer(${gv.id})">Xóa</button>
            ` : ''}
          </div>
        </div>
      `;
...
window.deleteLecturer = function(id) {
  const gv = DB.lecturers.find(l => l.id === id);
  if (!gv) return;
  if (confirm(`Bạn có chắc chắn muốn xóa giảng viên ${gv.name} (${gv.mgv})?`)) {
    DB.lecturers = DB.lecturers.filter(l => l.id !== id);
    DB.save();
    showToast(`Đã xóa giảng viên ${gv.name}`);
    renderLecturers();
  }
};
...
function renderTopics() {
  ...
  if (container) {
    ...
    const myGroup = isStudent ? getCurrentStudentGroup() : null;
    ...
    container.innerHTML = DB.topics.map(t => {
      ...
      if (isAdmin || isLecturer) {
        ...
        const registeredByMe = myGroup && myGroup.topicId === t.id;
        if (registeredByMe) {
          if (myGroup.status === "Pending") {
            actHtml = `
              <div style="display:flex; flex-direction:column; gap:4px; align-items:flex-end">
                <span class="badge badge-warning" style="font-size:0.72rem">Chờ duyệt đăng ký</span>
                <button class="btn btn-secondary btn-small" style="color:var(--danger); border-color:var(--danger); margin-top:4px" onclick="withdrawRegistration(${myGroup.id})">Rút đăng ký</button>
              </div>
            `;
          } else if (myGroup.status === "PendingCancel") {
            actHtml = `
              <span class="badge badge-danger" style="font-size:0.72rem">Chờ duyệt hủy</span>
            `;
          } else if (myGroup.status === "Active") {
            actHtml = `
              <div style="display:flex; flex-direction:column; gap:4px; align-items:flex-end">
                <span class="badge badge-success" style="font-size:0.72rem">Đã đăng ký</span>
                <button class="btn btn-secondary btn-small" style="color:var(--danger); border-color:var(--danger); margin-top:4px" onclick="requestCancelTopic(${myGroup.id})">Hủy đăng ký</button>
              </div>
            `;
          } else {
            actHtml = `
        ...
        } else if (!myGroup || myGroup.topicId === 0) {
...
window.registerTopic = function(id) {
  ...
  if (confirm(`Bạn muốn đăng ký thực hiện đề tài "${t.title}"?`)) {
    ...
    const myGroup = getCurrentStudentGroup();
    if (myGroup) {
      myGroup.topicId = t.id;
      myGroup.status = "Pending";
      myGroup.progress = 0;
      // Clear tasks for this group
      DB.tasks = DB.tasks.filter(tk => tk.groupId !== myGroup.id);
      showToast(`Đã đăng ký đồ án thành công! Đang chờ Giảng viên phê duyệt.`);
    } else {
      ...
      showToast(`Đã tạo nhóm và gửi yêu cầu đăng ký đề tài! Đang chờ Giảng viên phê duyệt.`);
    }
    ...
    DB.activities.unshift({
      id: newActId,
      text: `Sinh viên Phạm Minh Hoàng đã gửi yêu cầu đăng ký đề tài ${t.code}`,
      type: "system",
      time: "Vừa xong",
...
window.withdrawRegistration = function(groupId) {
  const grp = DB.groups.find(g => g.id === groupId);
  if (!grp) return;
  
  if (confirm("Bạn có chắc chắn muốn rút yêu cầu đăng ký đề tài này?")) {
    const oldTopicId = grp.topicId;
    const t = DB.topics.find(top => top.id === oldTopicId);
    grp.topicId = 0;
    grp.status = "Active";
    ...
    DB.activities.unshift({
      id: newActId,
      text: `Sinh viên Phạm Minh Hoàng đã rút yêu cầu đăng ký đề tài ${t ? t.code : ''}`,
      type: "system",
      time: "Vừa xong",
      user: "Phạm Minh Hoàng"
    });
    ...
    DB.save();
    showToast("Đã rút yêu cầu đăng ký đề tài thành công.");
    renderGroups();
    if (AppState.activeView === "topics") renderTopics();
  }
};

window.requestCancelTopic = function(groupId) {
  const grp = DB.groups.find(g => g.id === groupId);
  if (!grp) return;
  
  if (confirm("Bạn có chắc chắn muốn gửi yêu cầu hủy đăng ký đề tài này tới Giảng viên hướng dẫn?")) {
    grp.status = "PendingCancel";
    
    const t = DB.topics.find(top => top.id === grp.topicId);
    
    // Log activity
    const newActId = DB.activities.reduce((max, a) => a.id > max ? a.id : max, 0) + 1;
    DB.activities.unshift({
      id: newActId,
      text: `Sinh viên Phạm Minh Hoàng đã yêu cầu hủy đăng ký đề tài ${t ? t.code : ''}`,
      type: "system",
      time: "Vừa xong",
      user: "Phạm Minh Hoàng"
    });
    ...
    DB.save();
    showToast("Đã gửi yêu cầu hủy đăng ký đề tài. Đang chờ Giảng viên phê duyệt.");
    renderGroups();
    if (AppState.activeView === "topics") renderTopics();
  }
};
  ...
  if (listEl) {
    listEl.innerHTML = DB.groups.map(g => {
      ...
      const statusBadgeMap = {
        Pending: '<span class="badge badge-warning">Chờ GV duyệt đăng ký</span>',
        LecturerApproved: '<span class="badge badge-primary" style="background:rgba(59,130,246,0.15); border:1px solid var(--primary); color:var(--primary)">Chờ Hội đồng duyệt ĐK</span>',
        PendingCancel: '<span class="badge badge-danger">Chờ GV duyệt hủy</span>',
        LecturerCancelApproved: '<span class="badge badge-danger" style="background:rgba(239,68,68,0.15); border:1px solid var(--danger); color:var(--danger)">Chờ Hội đồng duyệt hủy</span>',
      ...
      const statusBadge = statusBadgeMap[g.status] || `<span class="badge badge-primary">${g.status}</span>`;
      ...
      if (isLecturer || isAdmin) {
        if (g.status === 'Pending') {
          buttonsHtml += `
            <button class="btn btn-primary btn-small" onclick="approveGroup(${g.id}, 'LecturerApproved')" style="background:var(--success); border-color:var(--success)">GV duyệt đăng ký</button>
          ...
          `;
        } else if (g.status === 'PendingCancel') {
          buttonsHtml += `
            <button class="btn btn-primary btn-small" onclick="approveCancelGroup(${g.id}, 'approve-gv')" style="background:var(--success); border-color:var(--success)">GV duyệt hủy</button>
            <button class="btn btn-secondary btn-small" style="color:var(--danger); border-color:var(--danger)" onclick="approveCancelGroup(${g.id}, 'reject-gv')">Từ chối hủy</button>
          `;
        }
      }

      if (isCommittee || isAdmin) {
        if (g.status === 'LecturerApproved') {
          buttonsHtml += `
            <button class="btn btn-primary btn-small" onclick="approveGroup(${g.id}, 'Active')" style="background:var(--success); border-color:var(--success)">Hội đồng duyệt đăng ký</button>
            <button class="btn btn-secondary btn-small" style="color:var(--danger); border-color:var(--danger)" onclick="approveGroup(${g.id}, 'Rejected')">Từ chối</button>
          `;
        } else if (g.status === 'LecturerCancelApproved') {
          buttonsHtml += `
            <button class="btn btn-primary btn-small" onclick="approveCancelGroup(${g.id}, 'approve-hd')" style="background:var(--success); border-color:var(--success)">Hội đồng duyệt hủy</button>
            <button class="btn btn-secondary btn-small" style="color:var(--danger); border-color:var(--danger)" onclick="approveCancelGroup(${g.id}, 'reject-hd')">Từ chối hủy</button>
          `;
        }
      }

      if (isStudent && g.leaderId === 1) { // Current mock student is leader
        if (g.status === 'Active' && g.topicId > 0) {
          buttonsHtml += `
            <button class="btn btn-secondary btn-small" style="color:var(--danger); border-color:var(--danger)" onclick="requestCancelTopic(${g.id})">Hủy đăng ký đề tài</button>
          `;
        } else if (g.status === 'Pending' || g.status === 'LecturerApproved') {
          buttonsHtml += `
            <button class="btn btn-secondary btn-small" onclick="withdrawRegistration(${g.id})">Rút yêu cầu đăng ký</button>
          `;
        }
      }
...
window.openLinkTopic = function(groupId) {
  ...
  if (choice) {
    ...
    if (idx >= 0 && idx < available.length) {
      const selectedTopic = available[idx];
      grp.topicId = selectedTopic.id;
      grp.status = "Pending"; // Set status to Pending for approval!
      grp.progress = 0;
      // Clear tasks for this group
      DB.tasks = DB.tasks.filter(tk => tk.groupId !== grp.id);
      
      // Log activity
      const newActId = DB.activities.reduce((max, a) => a.id > max ? a.id : max, 0) + 1;
      DB.activities.unshift({
        id: newActId,
        text: `Nhóm ${grp.name} đã đăng ký đề tài ${selectedTopic.code}`,
        type: "system",
        time: "Vừa xong",
        user: "Phạm Minh Hoàng"
      });
      ...
      DB.save();
      showToast(`Đã chọn đề tài và gửi yêu cầu phê duyệt!`);
      renderGroups();
    } else {
...
function renderProgress() {
  ...
  const isStudent = AppState.currentRole === "student";
  const selectWrap = document.getElementById("progress-group-select-wrap");
  let tasks = [];
  let group = null;
  ...
  if (isStudent) {
    ...
    // Show upload container for students
    const uploadBox = document.querySelector(".upload-container");
    if (uploadBox) uploadBox.style.display = "flex";
    const uploadHeader = document.querySelector("#progress-view h3");
    if (uploadHeader) uploadHeader.innerText = "Nộp Tài Liệu Báo Cáo";
    ...
    // Lecturer / Admin / Committee (Edit mode)
    if (selectWrap) selectWrap.style.display = "flex";
    
    // Hide upload container for reviewers
    const uploadBox = document.querySelector(".upload-container");
    if (uploadBox) uploadBox.style.display = "none";
    const uploadHeader = document.querySelector("#progress-view h3");
    if (uploadHeader) uploadHeader.innerText = "Danh Sách Tài Liệu Đồ Án";
    ...
    // Populate progress group select
    const select = document.getElementById("progress-group-select");
    if (select) {
      select.innerHTML = DB.groups.map(g => {
        const topic = DB.topics.find(t => t.id === g.topicId);
        const suffix = topic ? ` (${topic.code})` : " (Chưa có đề tài)";
        return `<option value="${g.id}">${g.name}${suffix}</option>`;
      }).join("");
      
      if (!AppState.progressGroupId) {
        AppState.progressGroupId = DB.groups[0]?.id || 1;
      }
      select.value = AppState.progressGroupId;
    }
    
    const activeGroupId = AppState.progressGroupId || 1;
    group = DB.groups.find(g => g.id === activeGroupId) || DB.groups[0];
    tasks = DB.tasks.filter(t => t.groupId === activeGroupId);
  }
  ...
  if (subContainer) {
    ...
    const groupSubmissions = DB.submissions.filter(s => s.groupId === targetGroupId);
    
    if (groupSubmissions.length === 0) {
      subContainer.innerHTML = `
        <div style="text-align:center; padding: 1.5rem; color:var(--text-muted); font-size:0.85rem">
          📁 Chưa nộp tài liệu nào.
        </div>
      `;
    } else {
      const isLecturer = AppState.currentRole === 'lecturer';
      ...
      subContainer.innerHTML = groupSubmissions.map(s => {
        let badgeClass = "badge-warning";
        let statusText = "Chờ giảng viên duyệt";
        if (s.status === "LecturerApproved") {
          badgeClass = "badge-primary";
          statusText = "Chờ Hội đồng duyệt";
          ...
          if (s.status === "Pending") {
            actionButtons = `
              <div style="display:flex; gap:6px; margin-top:8px">
                <button class="btn btn-primary btn-small" onclick="reviewSubmission(${s.id}, 'approve')" style="background:var(--success); border-color:var(--success); font-size:0.75rem; padding:4px 8px">Duyệt</button>
                <button class="btn btn-secondary btn-small" onclick="reviewSubmission(${s.id}, 'reject')" style="color:var(--danger); border-color:var(--danger); font-size:0.75rem; padding:4px 8px">Từ chối</button>
              </div>
            `;
          }
        }
        ...
        const logsHtml = s.reviewerLogs.map(log => `
        ...
        return `
          <div class="glass-card" style="padding: 1rem; margin-bottom: 0.75rem; border: 1px solid var(--border); display:flex; flex-direction:column; gap:6px">
            <div style="display:flex; justify-content:space-between; align-items:center; gap:8px">
              <span style="font-size:0.88rem; font-weight:600; color:var(--text-main); word-break:break-all">📄 ${s.filename}</span>
              <span class="badge ${badgeClass}" style="font-size:0.72rem; white-space:nowrap">${statusText}</span>
            </div>
            <div style="display:flex; justify-content:space-between; align-items:center; font-size:0.75rem; color:var(--text-muted)">
              <span>Thời gian: ${s.time}</span>
            </div>
...
window.handleFileSelected = function(e) {
  ...
  setTimeout(() => {
    ...
    const myGroup = getCurrentStudentGroup();
    if (!myGroup) {
      showToast("Bạn không thuộc nhóm đồ án nào!");
      return;
    }
    
    // Add to submissions
    const newSubId = DB.submissions.reduce((max, s) => s.id > max ? s.id : max, 0) + 1;
    const newSub = {
      id: newSubId,
      groupId: myGroup.id,
      filename: file.name,
      time: "Vừa xong",
      status: "Pending", // 'Pending' | 'LecturerApproved' | 'Approved' | 'Rejected'
      reviewerLogs: ["Sinh viên đã nộp bài"]
    };
    ...
    DB.activities.unshift({
      id: newActId,
      text: `Sinh viên Phạm Minh Hoàng đã tải lên tệp: ${file.name}`,
      type: "upload",
      time: "Vừa xong",
      user: "Phạm Minh Hoàng"
    });
    ...
    // Refresh progress view and dashboard if active
    if (AppState.activeView === "progress") renderProgress();
...
window.changeProgressGroup = function(val) {
  AppState.progressGroupId = parseInt(val) || 1;
  renderProgress();
};

window.reviewSubmission = function(subId, decision) {
  DB.load();
  ...
  const reviewerName = user.name.split(" (")[0];
  
  if (decision === 'approve') {
    if (role === 'lecturer' || role === 'admin') {
      if (sub.status === 'Pending') {
        sub.status = 'LecturerApproved';
        sub.reviewerLogs.push(`Giảng viên ${reviewerName} đã duyệt`);
        showToast(`Đã duyệt bài nộp "${sub.filename}" (GV). Chờ Hội đồng duyệt.`);
      }
    } else if (role === 'committee' || role === 'admin') {
      if (sub.status === 'LecturerApproved') {
        sub.status = 'Approved';
        sub.reviewerLogs.push(`Hội đồng ${reviewerName} đã duyệt`);
        showToast(`Hội đồng đã phê duyệt bài nộp "${sub.filename}" thành công!`);
      } else if (role === 'admin' && sub.status === 'Pending') {
        sub.status = 'LecturerApproved';
        sub.reviewerLogs.push(`Quản trị viên ${reviewerName} đã duyệt (GV)`);
        showToast(`Đã duyệt bài nộp "${sub.filename}" (GV). Chờ Hội đồng duyệt.`);
      }
    }
  } else if (decision === 'reject') {
    sub.status = 'Rejected';
    const roleText = role === 'lecturer' ? 'Giảng viên' : role === 'committee' ? 'Hội đồng' : 'Quản trị viên';
    sub.reviewerLogs.push(`${roleText} ${reviewerName} đã từ chối`);
    showToast(`Đã từ chối bài nộp "${sub.filename}"`);
  }
  
  // Log Activity
  const grp = DB.groups.find(g => g.id === sub.groupId) || { name: "Nhóm " + sub.groupId };
  const newActId = DB.activities.reduce((max, a) => a.id > max ? a.id : max, 0) + 1;
  DB.activities.unshift({
    id: newActId,
    text: `${reviewerName} đã ${decision === 'approve' ? 'phê duyệt' : 'từ chối'} tài liệu ${sub.filename} của ${grp.name}`,
    type: "system",
    time: "Vừa xong",
    user: reviewerName
  });
...
window.changeScoringGroup = function(val) {
  AppState.scoringGroupId = parseInt(val) || 1;
  renderScoring();
};
...
function renderScoring() {
  ...
  const isStudent = AppState.currentRole === "student";
  const mainContent = document.getElementById("scoring-main-content");
  const pendingState = document.getElementById("scoring-student-pending-state");
  const noGroupState = document.getElementById("scoring-student-no-group-state");
  const selectWrap = document.getElementById("scoring-group-select-wrap");
  const actionsContainer = document.getElementById("scoring-actions-container");
  ...
  if (isStudent) {
    ...
    if (!myGroup) {
      ...
      if (noGroupState) noGroupState.style.display = "flex";
      return;
    }
    
    const sc = DB.scores.find(s => s.groupId === myGroup.id);
    if (!sc || !sc.approved) {
      if (mainContent) mainContent.style.display = "none";
      if (pendingState) pendingState.style.display = "flex";
      if (noGroupState) noGroupState.style.display = "none";
      return;
    }

    // Show scoring details (read-only for student)
    if (mainContent) mainContent.style.display = "";
    if (pendingState) pendingState.style.display = "none";
    if (noGroupState) noGroupState.style.display = "none";

    document.querySelectorAll(".rubric-slider").forEach(el => el.disabled = true);
    ...
    document.querySelectorAll(".rubric-slider").forEach(el => el.disabled = false);
    const ta = document.getElementById("scoring-comment-area");
    if (ta) ta.disabled = false;

    // Populate group select dropdown
    const select = document.getElementById("scoring-group-select");
    if (select) {
      select.innerHTML = DB.groups.map(g => {
        const topic = DB.topics.find(t => t.id === g.topicId);
        const suffix = topic ? ` (${topic.code})` : " (Chưa có đề tài)";
      ...
      if (!AppState.scoringGroupId) {
        AppState.scoringGroupId = DB.groups[0]?.id || 1;
      }
      select.value = AppState.scoringGroupId;
    }

    const targetGroupId = AppState.scoringGroupId;
    const sc = DB.scores.find(s => s.groupId === targetGroupId) || { progress: 0, report: 0, code: 0, presentation: 0, comments: "", approved: false };
...
function calculateFinalScore() {
  ...
  if (scoreCard) {
    ...
    if (resultLabel) {
      let badgeHtml = "";
      if (roundFinal >= 8.5) badgeHtml = `<span class="badge badge-success">Xuất sắc (A)</span>`;
      else if (roundFinal >= 7.0) badgeHtml = `<span class="badge badge-primary">Khá (B)</span>`;
      else if (roundFinal >= 5.0) badgeHtml = `<span class="badge badge-warning">Đạt trung bình (C)</span>`;
      else badgeHtml = `<span class="badge badge-danger">Không đạt (F)</span>`;
      
      const isStudent = AppState.currentRole === "student";
      if (!isStudent) {
        const targetGroupId = AppState.scoringGroupId || 1;
        const sc = DB.scores.find(s => s.groupId === targetGroupId);
        const isApproved = sc && sc.approved;
        if (isApproved) {
          badgeHtml += `<div style="margin-top:10px; display:flex; align-items:center; justify-content:center; gap:4px"><span class="badge badge-success" style="font-size:0.75rem; background:rgba(34,197,94,0.15); border:1px solid var(--success); color:var(--success)">✓ Đã công bố cho SV</span></div>`;
        } else {
          badgeHtml += `<div style="margin-top:10px; display:flex; align-items:center; justify-content:center; gap:4px"><span class="badge badge-warning" style="font-size:0.75rem; background:rgba(249,115,22,0.15); border:1px solid var(--primary); color:var(--primary)">⚠ Bản nháp (Chưa công bố)</span></div>`;
        }
      } else {
        badgeHtml += `<div style="margin-top:10px; display:flex; align-items:center; justify-content:center; gap:4px"><span class="badge badge-success" style="font-size:0.75rem; background:rgba(34,197,94,0.15); border:1px solid var(--success); color:var(--success)">✓ Điểm chính thức</span></div>`;
      }
      resultLabel.innerHTML = badgeHtml;
    }
  }
}

window.saveDefenseScores = function(approved = false) {
  if (AppState.currentRole === "student") return;
  ...
  const scoreIndex = DB.scores.findIndex(s => s.groupId === targetGroupId);
  const scoreObj = { 
  ...
  // Log Activity
  const grp = DB.groups.find(g => g.id === targetGroupId) || { name: "Nhóm " + targetGroupId };
  const actionText = approved ? "đã phê duyệt & công bố điểm" : "đã lưu bản nháp điểm";
  const newActId = DB.activities.reduce((max, a) => a.id > max ? a.id : max, 0) + 1;
  DB.activities.unshift({
    id: newActId,
    text: `${USER_PROFILES[AppState.currentRole].name.split(" (")[0]} ${actionText} cho ${grp.name}: ${final} điểm`,
    type: "score",
    time: "Vừa xong",
  ...
  if (approved) {
    showToast(`Đã phê duyệt và công bố điểm cho ${grp.name}: ${final} / 10!`);
  } else {
    showToast(`Đã lưu bản nháp điểm cho ${grp.name} thành công.`);
  }
...
window.approveGroup = function(id, status) {
  ...
  const t = DB.topics.find(top => top.id === g.topicId);
  const topicCode = t ? t.code : '';
  const user = USER_PROFILES[AppState.currentRole];
  const userName = user ? user.name.split(" (")[0] : "Người dùng";
  
  if (status === 'LecturerApproved') {
    g.status = "LecturerApproved";
    
    const newActId = DB.activities.reduce((max, a) => a.id > max ? a.id : max, 0) + 1;
    DB.activities.unshift({
      id: newActId,
      text: `${userName} đã duyệt đăng ký đề tài ${topicCode} cho nhóm ${g.name}, chờ Hội đồng duyệt`,
      type: "system",
      time: "Vừa xong",
      user: userName
    });
    
    showToast(`Giảng viên đã duyệt đăng ký đề tài cho nhóm ${g.name}!`);
  } else if (status === 'Active') {
    g.status = "Active";
    if (t) t.status = "Đang thực hiện";
    
    const newActId = DB.activities.reduce((max, a) => a.id > max ? a.id : max, 0) + 1;
    DB.activities.unshift({
      id: newActId,
      text: `${userName} đã phê duyệt đề tài ${topicCode} cho nhóm ${g.name}`,
      type: "system",
      time: "Vừa xong",
      user: userName
    });
    
    showToast(`Hội đồng đã phê duyệt đề tài cho nhóm ${g.name}!`);
  } else if (status === 'Rejected') {
    // Clear topicId and reset status back to Active
    g.topicId = 0;
    g.status = "Active";
    ...
    DB.activities.unshift({
      id: newActId,
      text: `${userName} đã từ chối yêu cầu đăng ký đề tài ${topicCode} của nhóm ${g.name}`,
      type: "system",
      time: "Vừa xong",
      user: userName
    });
    
    showToast(`Đã từ chối đăng ký đề tài của nhóm ${g.name}`);
  }
...
window.approveCancelGroup = function(id, decision) {
  const g = DB.groups.find(grp => grp.id === id);
  if (!g) return;
  ...
  if (decision === 'approve-gv') {
    g.status = "LecturerCancelApproved";
    
    const newActId = DB.activities.reduce((max, a) => a.id > max ? a.id : max, 0) + 1;
    DB.activities.unshift({
      id: newActId,
      text: `${userName} đã duyệt yêu cầu hủy đề tài ${topicCode} của nhóm ${g.name}, chờ Hội đồng duyệt`,
      type: "system",
      time: "Vừa xong",
      user: userName
    });
    
    showToast(`Giảng viên đã duyệt yêu cầu hủy đề tài cho nhóm ${g.name}!`);
  } else if (decision === 'approve-hd') {
    g.topicId = 0;
    g.status = "Active";
    ...
    DB.activities.unshift({
      id: newActId,
      text: `${userName} đã phê duyệt yêu cầu hủy đề tài ${topicCode} của nhóm ${g.name}`,
      type: "system",
      time: "Vừa xong",
      user: userName
    });
    
    showToast(`Hội đồng đã phê duyệt hủy đề tài cho nhóm ${g.name}!`);
  } else if (decision === 'reject-gv' || decision === 'reject-hd') {
    g.status = "Active"; // remains active, topic stays
    
    const newActId = DB.activities.reduce((max, a) => a.id > max ? a.id : max, 0) + 1;
    DB.activities.unshift({
      id: newActId,
      text: `${userName} đã từ chối yêu cầu hủy đề tài ${topicCode} của nhóm ${g.name}`,
      type: "system",
      time: "Vừa xong",
      user: userName
    });
    
    showToast(`Đã từ chối yêu cầu hủy đề tài của nhóm ${g.name}`);
  }
